...
static int ap3xx6_Calibration_every_time(struct i2c_client *client)
{
    ...
    if(abs(ui_ps_data - gi_ps_raw_data) > DI_PS_CAL_DIFF)
    {
    	...
    	ap3xx6_read_ps_cal(obj->client, &ps_cal);
    	msleep(50);
    	
        if((gi_ps_raw_data + ps_cal) > 511 )
        {
      	...
      	{
      		ap3xx6_set_pcrosstalk(obj->client, (gi_ps_raw_data + ps_cal));
      	}
...
static ssize_t ap3426_Huaqin_show_val(struct device_driver *ddri, char *buf)
{
	...
	for (i = 0; i < 2; ++i)
	{
		if((ap3xx6_read_ps(ap3xx6_obj->client, &ps_value)) != 0)
		{
			APS_ERR("ap3xx6_read_ps fail\n");
			return -1;
		}
	...
	ps_value = ps_count/2;
	count = sprintf(buf,"AP3426 PS val=%d\n",reg_value[0] + (reg_value[1]<<8) + ps_value);
	return count;
}
//static DRIVER_ATTR(em,	S_IWUGO | S_IRUGO, ap3xx6_em_read, ap3xx6_em_write);
static DRIVER_ATTR(Huaqin_show_val,	S_IWUSR | S_IRUGO, ap3426_Huaqin_show_val, NULL);
