...
#include "imx2191mipiraw_Sensor.h"
...
extern int IMX2191_UpdateLSC(void);
extern int read1_otp_module_id(void);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = IMX2191_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
#define IMX2191MIPI_MaxGainIndex (97)
kal_uint16 IMX2191MIPI_sensorGainMapping[IMX2191MIPI_MaxGainIndex][2] ={
{ 64 ,0  },   
{ 68 ,12 },   
{ 71 ,23 },   
{ 74 ,33 },   
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    printk("[IMX2191MIPI]enter IMX219MIPIGain2Reg function\n");
    for (iI = 0; iI < (IMX2191MIPI_MaxGainIndex-1); iI++) 
	{

        if(gain <IMX2191MIPI_sensorGainMapping[iI][0])
		{    
            break;
        }
		if(gain == IMX2191MIPI_sensorGainMapping[iI][0]) //20140718 Jeff mtk07693 change from < to ==
		{                
			return IMX2191MIPI_sensorGainMapping[iI][1];       
		}
    ...
    if(gain != IMX2191MIPI_sensorGainMapping[iI][0])
    {
         printk("Gain mapping don't correctly:%d %d \n", gain, IMX2191MIPI_sensorGainMapping[iI][0]);
    }
	printk("exit IMX2191MIPIGain2Reg function\n");
    return IMX2191MIPI_sensorGainMapping[iI-1][1];  //here,if gain < 64, stack overflow happen.
	//return NONE;
...
//
kal_uint8  test_pattern_flag1=0;
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		module_id = read1_otp_module_id();
		printk("ofg_module_id %d\n",module_id);
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {		
				printk("imx219_mipi_raw_ofg_zaw875:[imx219]i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			printk("imx219_mipi_raw_ofg_zaw875:[imx219]Read sensor id fail, i2c write id: 0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	//if (imgsensor_info.sensor_id != sensor_id  || module_id != 0x07 )
