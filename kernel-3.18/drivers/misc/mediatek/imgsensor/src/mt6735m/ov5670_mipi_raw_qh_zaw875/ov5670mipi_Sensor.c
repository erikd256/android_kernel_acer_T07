...
*
*************************************************************************/
static unsigned sub_camera_gpio_id_pin = (9);
...
//sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
...
#if 1		//WENDELL-20160119 added for front camera id pin
		l_ret = gpio_request(sub_camera_gpio_id_pin,"front_camera_id_pin");
		if (l_ret < 0) {
			printk("l_ret = %d\n" ,l_ret);
			printk("Unable to request gpio front_camera_id_pin\n");
			gpio_free(MTK_PIN_NO(9));
			//return -1;
		}
		else{
			l_ret = gpio_direction_input(sub_camera_gpio_id_pin);
			printk("[WENDELL]-%s: r_ret = %d\n", __func__, l_ret);
		}	
#endif
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		printk("fmx quhun ov5670 \n");
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id &&(0==gpio_get_value(sub_camera_gpio_id_pin))) {
				printk("fmx quhun ov5670 \n");
