...
void kpd_slide_qwerty_init(struct input_dev *dev)
{
   ...
   kpd_print("[slide]%s,%d",__func__,__LINE__);

   ret = request_irq(g_slide_dev.slide_irq,(irq_handler_t)kpd_slide_eint_handler,g_slide_dev.irq_flag,"slide-eint",NULL);

            if(ret != 0)
   kpd_print("[slide]:kpd request_irq fail ret=%d irq=%d,flag=%d\r\n",ret,g_slide_dev.slide_irq,g_slide_dev.irq_flag);
...
#ifdef CONFIG_LONGPRESS_KEY_POWEROFF
